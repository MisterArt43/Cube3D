...
void	load_texture(t_game *game, t_data *texture, int *i)
{
	...
	*i += 3;
	skip_to_next_word(game->map.file_str, i);
	sub = *i;
	skip_to_EOL_or_EOF(game->map.file_str, &sub);
	while ((game->map.file_str[sub] == ' ' || \
	game->map.file_str[sub] == '\t') && sub > *i)
		sub--;
	*texture = init_texture(game, ft_substr(game->map.file_str, *i, sub - *i));
}
...
void	sort_parse(t_game *game)
{
	...
	i = 0;
	type = 0;
	while (game->map.file_str[i])
	{
		skip_to_next_word(game->map.file_str, &i);
		if (game->map.file_str[i] == '\n')
		{
			i++;
			define_type(game->map.file_str, i, &type);
			if (type == 1)
...
void	start_parse(char *map_file, t_game *game)
{
	...
	game->map.file_str = ft_readall(game->map.fd, game);
	close(game->map.fd);
	if (!game->map.file_str)
