...
t_game	*game_initializer(char *map_path)
{
	...
	game->mlx = mlx_init(); // attention mlx peut renvoyer NULL !!!!!!!!
	if (!game->mlx)
	...
	//game->game_tab = tmp_game_tab_feeder(map_path, game);//old one
	...
	game->raycast = raycast_initializer(); //malloc à protéger
